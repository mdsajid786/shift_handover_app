...
def escalation_matrix():
    ...
    if files:
        ...
        # Super Admin: only show files for selected account/team if filter is applied
        if current_user.role == 'super_admin':
            if account_id is not None and team_id is not None:
                file_query = file_query.filter_by(account_id=account_id, team_id=team_id)
            ...
            if latest_db_file:
                ...
                app_names = []
        # Account Admin: only show files for their account and selected team
        elif current_user.role == 'account_admin':
            if team_id is not None:
                file_query = file_query.filter_by(account_id=current_user.account_id, team_id=team_id)
            else:
            ...
            if latest_db_file:
                ...
                app_names = []
        # Team Admin: only show files for their account/team
        else:
